...
const ModuleCard: React.FC<ModuleCardProps> = ({
    ...
    return (
        ...
        >
            <div
            className={`group rounded-[28px] overflow-hidden bg-white dark:bg-slate-900/90 backdrop-blur-xl border transition-all duration-500 flex flex-col h-full animate-fade-in ${
                isCompleted
                    ? 'border-amber-300/80 dark:border-amber-500/50 shadow-[0_15px_35px_-10px_rgba(245,158,11,0.25)]'
                    : 'border-white/90 dark:border-slate-800 shadow-[0_10px_30px_-5px_rgba(15,23,42,0.06)] hover:shadow-2xl hover:border-sky-300/60 dark:hover:border-sky-500/50 hover:-translate-y-1.5'
            }`}
        ...
        >
            ...
            <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3.5 right-4 flex items-center gap-1.5 font-extrabold shadow-md backdrop-blur-md transition-all">
                    {isCompleted ? (
                        ...
                        <div className="bg-white/90 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs px-3 py-1 rounded-full border border-slate-200/80 dark:border-slate-700">
            ...
            {/* Corpo do Card */}
            <div className="p-6 flex flex-col flex-1 bg-white/95 dark:bg-slate-900/95">
                ...
                {/* Título do Módulo */}
                <h3 className="font-extrabold text-xl text-[#0f172a] dark:text-slate-50 mb-2 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors leading-snug">
                    {title}
                </h3>

                {/* Descrição */}
                <p className="text-slate-600 dark:text-slate-300 text-xs sm:text-sm leading-relaxed font-light mb-6 flex-grow line-clamp-3">
                    {description}
                </p>
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                    {resources.map((res, i) => (
                        <span 
                            key={i}
                            className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-sky-50 dark:bg-sky-950/60 text-sky-800 dark:text-sky-300 border border-sky-200/70 dark:border-sky-800/60"
                        >
                            {res}
                        </span>
                    ))}
