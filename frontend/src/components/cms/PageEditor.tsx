...
import {
    ...
    ExternalLink, Link as LinkIcon, History, BookOpen, Eye, EyeOff
} from 'lucide-react';
...
const [mobileActiveDrawer, setMobileActiveDrawer] = useState<'blocks' | 'properties' | 'ai' | 'media' | 'history' | 'configs' | null>(null);
...
<button 
    ...
    className="p-1.5 sm:p-2 text-warm-500 hover:text-warm-900 dark:text-slate-400 dark:hover:text-white hover:bg-warm-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
>
<ArrowLeft size={16} />
</button>
<div className="h-5 w-px bg-warm-200 dark:bg-slate-800 hidden sm:block" />
<select
value={selectedPage}
...
<button
    ...
    className={`hidden md:flex px-2.5 sm:px-3 py-1.5 text-xs font-bold rounded-lg transition-all items-center gap-1.5 whitespace-nowrap cursor-pointer ${
        rightSidebarTab === 'ai' 
...
>
    <Save size={14} /> <span className="hidden xs:inline">Publicar</span><span className="xs:hidden">Salvar</span>
</button>
</div>
</div>
...
<div className={`transition-all duration-300 ${
    deviceView === 'desktop' 
        ? 'max-w-6xl w-full flex flex-col gap-4 pb-32'
        : deviceView === 'tablet'
...
{/* Desktop Order Controls — visible on hover or select */}
<div className={`hidden md:flex absolute -left-10 top-1/2 -translate-y-1/2 flex-col gap-1 transition-opacity ${
...
<div className="flex flex-col items-center justify-center p-12 sm:p-20 border-2 border-dashed border-warm-300 rounded-3xl bg-white/60 text-warm-500">
<Layers size={48} className="mb-4 opacity-40" />
<p className="text-base sm:text-lg font-medium">Sua página está vazia.</p>
</div>
)}
</div>
</div>
...
<div className={`hidden md:flex flex-col z-40 shrink-0 shadow-lg bg-white dark:bg-slate-900 border-l border-warm-200 dark:border-slate-800 transition-all duration-300 ${
    ...
    <div className="flex items-center border-b border-warm-200 dark:border-slate-800 bg-warm-50/70 dark:bg-slate-800/80 p-1.5 gap-1 shrink-0">
        <button 
            ...
            className={`flex-1 py-1.5 px-1 text-xs font-bold rounded-lg flex items-center justify-center gap-1 transition-all ${
                    ...
                    <div className="flex items-center gap-2">
                        {mobileActiveDrawer === 'blocks' && (
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-extrabold text-sm text-warm-900 dark:text-white">Adicionar Novo Bloco na Página</h3>
                            </div>
                        )}
                        ...
                        {mobileActiveDrawer === 'ai' && (
                            <div className="flex items-center gap-2">
                                <Wand2 size={18} className="text-purple-600" />
                                <h3 className="font-extrabold text-sm text-warm-900 dark:text-white">Criador de Páginas com IA</h3>
                            </div>
                        )}
                ...
                <div className="p-4 overflow-y-auto flex-1 custom-scrollbar pb-10">
                    {mobileActiveDrawer === 'blocks' && (
                        <div className="grid grid-cols-1 gap-2.5 pb-6">
                            <p className="text-xs text-warm-500 mb-1">Toque em qualquer recurso abaixo para inseri-lo na página:</p>
                            {BLOCK_TEMPLATES.map(tmpl => (
                                <button
                                    key={tmpl.type}
                                    type="button"
                        ...
                        <div className="pb-6">
                            {!selectedBlock ? (
                                <div className="text-center py-6 space-y-3">
                                    <p className="text-xs text-warm-500">Selecione um bloco da página para editar:</p>
                        ...
                        <div className="space-y-3 pb-6">
                            <div className="bg-purple-50 border border-purple-200 p-3 rounded-2xl">
                                <p className="text-xs font-bold text-purple-900 mb-1 flex items-center gap-1.5">
                                    <Sparkles size={14} className="text-purple-600" /> Agente IA de Criação de Páginas
                                </p>
                                <p className="text-[11px] text-purple-700">Digite o que deseja construir nesta página para gerar o layout com IA.</p>
                            </div>
                            <textarea
                                value={aiPrompt}
                                onChange={(e) => setAiPrompt(e.target.value)}
                                placeholder="Ex: Crie uma seção de apresentação da equipe de cuidados paliativos..."
                    ...
                    {mobileActiveDrawer === 'media' && (
                        <div className="space-y-3 pb-6">
                            <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-2xl">
                                ...
                                <p className="text-[11px] text-emerald-700">Pesquise fotos de saúde e cuidados paliativos para ilustrar sua página.</p>
                            </div>
                            <div className="flex gap-2">
                                <input
                                    type="text"
